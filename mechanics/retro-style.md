...
`RetroLoot` is an option I want to give in the settings menu, which does the following: 
* All freely placed items in the world (no shop items) are transformed to `FloatingItems` which are floating a bit above the ground, they may or may not also slowly rotate around 360° like in the inventory or this may be reserved for special items, e.g. `MissionItems`.
* `FloatingItems` are surrounded by a slight light effect; effect style and/or colour may depend on category (e.g. healing potions omit red light etc.)
...
* Containers (chests, crates, npcs etc.) are looted as usual by pressing the action button when in focus, upon which the containers inventory appears.
...
The standard option is called `ImmersiveLoot`, where all items remain carefully placed by hand at their respective position and are taken by accompanying animations.
